...
int main(int argc, char** argv) {
    ...
    if (argc > 1) { 
        for (int i = 1; i < argc; ++i) { 
            ...
            std::cout << "pushing arg -> " << argv[i] << std::endl;
        }
        
        auto no_gui_found = std::find(program_args.begin(), program_args.end(), "--no-gui");
        draw_gui = no_gui_found == program_args.end(); // if flag no-gui flag is found, this will return false, and not draw the window
    }
